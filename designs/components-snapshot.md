...
2. **Jurisdiction chip** — `juris-chip`, two-letter code (`US` / `TW` / `EU` / `JP` / `CH`) per [`primitives.md`](./primitives.md) §Chip family / Jurisdiction chip. The full region name is **not** repeated in the row — the chip carries the signal and the industry / context that follows is the human-readable text.
...
- **Variants:** default; `pillar-meta-juris` (markets sub-stat with "N markets · US TW EU JP CH" two-row variant)
- **States:** rest
...
**Body copy.** `flex: 1; min-width: 0`. Single `<p>`: Urbanist `20 / 500 / -0.005em / 1.45`. Flat silver register — `color: var(--slate-700)` (light) / `var(--slate-200)` on `[data-theme="dark"]`. The former clipped text-gradient treatment was retired with the gradient system ([`design-tokens.md`](./design-tokens.md) §7.5); line-height stays at 1.45 because it reads better at this size, not because of `background-clip` descender clearance.
