...
# Patent Preview (Licensing sub-page)

> **Source:** [website/product/licensing/patent-preview.html](../product/licensing/patent-preview.html) · new page added 2026-06-01 · static + inline-JS, no auth.
> Acts as the public proof-of-service before the lobby sign-up gate. Browseable 120-patent mock inventory with filters; real inventory swaps in by replacing the inline `PATENTS` array.

## Top nav

> **Canonical content lives at [Homepage › Top nav](#top-nav).** This page mirrors the canonical Top nav with **one divergence**: the CTA pill is `查看定價 / View pricing` (linking to `/product/licensing/#pricing`), not the canonical `聯絡業務 / Contact sales`. The divergence is tracked below; all other rows match canonical.
...
> Same divergence as Top nav — the `mobile-auth` row carries `查看定價 / View pricing` instead of canonical `聯絡業務 / Contact sales`. Single-row delta tracked in Top nav above.

## Search modal

> **1:1 with [Homepage › Search modal](#search-modal).** Byte-identical. Edits land in all 4 files now (homepage + 2 product pages + this preview).

## IP intelligence drop popup

> **Intentionally NOT present on this page.** Pop-up overlay would interrupt a browse-focused surface. Lobby pages also skip it (precedent).

## H1 (screen-reader only)

| ZH | ZH ✓ | Literal EN | Site EN | Status |
|---|---|---|---|---|
| 專利庫存 — 瀏覽 1,000+ 件可授權專利 | ☐ | Patent inventory — browse 1,000+ licensable patents | Patent inventory — browse 1,000+ available patents | 🆕 ☐ |
...
|---|---|---|---|---|
| 產業 |  | Industry | Industry | ☐ |
| 司法管轄區 |  | Jurisdiction | Jurisdiction | ☐ |
| 評等 |  | Rating / Grade | Tier | ☐ |
| 引用 |  | Citations | Cites | ☐ |
| 家族 |  | Family | Family | ☐ |
| 申請年 |  | Filing year | Filed | ☐ |
| 排序 |  | Sort | Sort | ☐ |
| 清除 |  | Clear | Clear | ☐ |
| 件顯示 |  | items shown | shown | ☐ |

## Filter bar — Industry options

| ZH | ZH ✓ | Literal EN | Site EN | Status |
|---|---|---|---|---|
| 所有產業 |  | All industries | All industries | ☐ |
| 電子元件 |  | Electronic components | Electronic Components | ☐ |
| 機械設計 |  | Mechanical design | Mechanical Design | ☐ |
| 綠能 |  | Green energy | Green Power | ☐ |
| 半導體 |  | Semiconductor | Semiconductors | ☐ |
| 顯示技術 |  | Display technology | Display Technology | ☐ |
| 生醫 |  | Biomedical | Biomedical | ☐ |
| 通訊 |  | Communications | Communications | ☐ |
| 軟體與 AI |  | Software and AI | Software & AI | ☐ |
| 材料科學 |  | Materials science | Materials Science | ☐ |
| 光學與光電 |  | Optics and photonics | Optics & Photonics | ☐ |
| 儲能 |  | Energy storage | Energy Storage | ☐ |
| 機器人 |  | Robotics | Robotics | ☐ |

## Filter bar — Jurisdiction options

| ZH | ZH ✓ | Literal EN | Site EN | Status |
|---|---|---|---|---|
| 所有地區 |  | All regions | All jurisdictions | ☐ |
| 美國 |  | United States | United States | ☐ |
| 台灣 |  | Taiwan | Taiwan | ☐ |
| 歐盟 |  | EU | Europe | ☐ |
| 日本 |  | Japan | Japan | ☐ |
...
## Filter bar — threshold dropdowns

| ZH | ZH ✓ | Literal EN | Site EN | Status |
|---|---|---|---|---|
| 不限 |  | Unrestricted / Any | Any | ☐ |
| 10 + |  | 10 + | 10 + | ☐ |
| 25 + |  | 25 + | 25 + | ☐ |
| 50 + |  | 50 + | 50 + | ☐ |
| 3 + |  | 3 + | 3 + | ☐ |
| 5 + |  | 5 + | 5 + | ☐ |
| 10 + |  | 10 + | 10 + | ☐ |
| 近 2 年 |  | Last 2 years | Last 2 yrs | ☐ |
| 近 5 年 |  | Last 5 years | Last 5 yrs | ☐ |
| 近 10 年 |  | Last 10 years | Last 10 yrs | ☐ |

## Filter bar — Sort options

| ZH | ZH ✓ | Literal EN | Site EN | Status |
|---|---|---|---|---|
| 評等 S → D |  | Rating S → D | By tier (S → D) | ☐ |
| 引用 高 → 低 |  | Citations high → low | By citations | ☐ |
| 家族 高 → 低 |  | Family high → low | By family size | ☐ |
| 申請日 新 → 舊 |  | Filing date new → old | By filing date | ☐ |

## Dynamic strings (JS-rendered, not data-zh)

> Patent card meta (`<assignee> · <year> · <industry name>`, `<IPC> · N cites · N family`) and pagination labels (Prev / Next, empty state) are emitted by the inline `<script>` based on `document.documentElement.lang`. EN/ZH are baked into the JS, not into `data-zh` attributes. Maintenance: edit the script's `isZh()` branches, not this audit doc.

| Source | ZH ✓ | EN form | ZH form | Status |
|---|---|---|---|---|
| Card meta — citations word | ☐ | `cites` | `次引用` | 🆕 ☐ |
| Card meta — family word | ☐ | `family` | `件家族` | 🆕 ☐ |
| Pagination — Prev | ☐ | `Prev` | `上一頁` | 🆕 ☐ |
| Pagination — Next | ☐ | `Next` | `下一頁` | 🆕 ☐ |
| Empty state — heading | ☐ | `No patents match your filters` | `沒有符合篩選的專利` | 🆕 ☐ |
| Empty state — body | ☐ | `Try clearing some filters or expanding your jurisdiction.` | `請放寬產業、司法管轄區或評等條件。` | 🆕 ☐ |

## Bottom CTA band

| ZH | ZH ✓ | Literal EN | Site EN | Status |
|---|---|---|---|---|
| 找到適合你的專利？ | ☐ | Found patents that suit you? | See patents you can use? | 🆕 ☐ |
| 訂閱 30 件專利組合，依司法管轄區與產業組合。NT$8,990/月起，3 個月起跳。 | ☐ | Subscribe to a 30-patent bundle, by jurisdiction × industry combination. From NT$8,990/month, 3-month minimum. | Subscribe to a curated 30-patent bundle in your jurisdiction and industry. From NT$8,990/month, 3-month minimum. | 🆕 ☐ |
| 查看定價 → | ☐ | View pricing → | View pricing → | 🆕 ☐ |
| 聯絡業務 |  | Contact sales | Talk to sales | ☐ |

## Footer

> **Canonical content lives at [Homepage › Footer](#footer).** Same byte-for-byte text content as the canonical Footer (only `href` plumbing differs — links to `/#about`, `/product/licensing/#contact` etc. follow the same pattern as the other product pages). Edits land in all 4 files now.
